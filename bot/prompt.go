...
func (hs *HabitMessage) HabitMsgs(h Habit, category string) {
	...
	case WelcomeOnStartCommad:
		hs.StartCommandMsgs.WelcomeMsg = fmt.Sprintf("Generate an exciting and motivating welcome message for a Telegram habit-making bot. This message should be two-to-3 lines long, filled with positive energy, and include fun emojis. It should warmly welcome the user, %s, highlighting that this bot is designed to help groups build lasting habits together. Stress that the first step is to add the bot to their group by clicking the link below: https://t.me/MoneyMngmnt_Bot?startgroup=true..%s", h.Name, hs.Base.AllowedSymobls)
	}
}
